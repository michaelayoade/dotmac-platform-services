...
try:
    from dotmac.platform.communications.email_service import (
        EmailMessage, EmailResponse, EmailService, get_email_service, send_email
    )
...
class TestTemplateService:
    ...
    def test_render_template_success(self):
        ...
        rendered = service.render_template(created.id, {
            "name": "John",
            "company": "Acme Corp"
        })
    ...
    def test_list_and_delete_templates(self):
        ...
        # Create templates
        template1 = service.create_template(TemplateData(
            name="template1", subject_template="Subject 1"
        ))
        template2 = service.create_template(TemplateData(
            name="template2", subject_template="Subject 2"
        ))
...
class TestTaskService:
    ...
    def test_queue_bulk_emails_convenience(self, mock_task):
        ...
        messages = [
            EmailMessage(to=["user@example.com"], subject="Test", text_body="Hello")
        ]
...
class TestIntegration:
    ...
    async def test_template_to_email_flow(self):
        ...
        # Render template
        rendered = template_service.render_template(template.id, {
            "name": "John Doe",
            "company": "Test Corp"
        })
    ...
    def test_error_handling_across_services(self):
        ...
        with pytest.raises(ValueError, match="Syntax error"):
            template_service.create_template(TemplateData(
                name="bad",
                subject_template="Bad {{syntax"  # Missing closing brace
            ))
