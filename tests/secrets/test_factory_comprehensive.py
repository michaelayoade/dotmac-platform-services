...
class TestSecretsManagerFactory:
    ...
    def test_create_vault_with_dependency_error(self, mock_dep_checker, mock_settings):
        ...
        mock_settings.features.secrets_vault = True
        mock_dep_checker.require_feature_dependency.side_effect = DependencyError("Missing vault dependencies")
    ...
    def test_auto_select_with_none_backend(self, mock_create):
        """Test that None backend triggers auto-selection."""
        mock_create.return_value = MagicMock()

        with patch.object(SecretsManagerFactory, '_auto_select_backend', return_value='local') as mock_auto:
            manager = SecretsManagerFactory.create_secrets_manager(None, test_param="value")

            mock_auto.assert_called_once()
            mock_create.assert_called_with('local', test_param="value")
...
class TestConvenienceFunctions:
    ...
    @patch("dotmac.platform.secrets.factory.SecretsManagerFactory.create_secrets_manager")
    def test_create_vault_secrets_manager_function(self, mock_create):
    ...
    def test_create_vault_manager_decorator(self, mock_decorator):
        ...
        assert hasattr(create_vault_secrets_manager, '__wrapped__') or callable(create_vault_secrets_manager)
