...
def create_mock_activity(activity_type, user_id="user1", resource_id="secret1", 
                         timestamp=None, details=None, action="secret_access"):
...
class TestSecretsMetricsEndpointWithMocks:
    ...
    async def test_endpoint_success_with_activities(self, app_with_router):
        ...
        mock_activities = [
            create_mock_activity(ActivityType.SECRET_ACCESSED, "user1", "db/password", now - timedelta(days=1)),
            create_mock_activity(ActivityType.SECRET_ACCESSED, "user1", "db/password", now - timedelta(days=2)),
            create_mock_activity(ActivityType.SECRET_ACCESSED, "user2", "api/key", now - timedelta(days=3)),
        ]
        ...
        async with AsyncClient(transport=ASGITransport(app=app_with_router), base_url="http://test") as client:
    ...
    async def test_endpoint_with_crud_operations(self, app_with_router):
        ...
        def mock_execute(query):
            ...
            if "SECRET_ACCESSED" in query_str:
                ...
                activities = [
                    create_mock_activity(ActivityType.SECRET_CREATED, timestamp=now - timedelta(days=1)),
                    create_mock_activity(ActivityType.SECRET_CREATED, timestamp=now - timedelta(days=2)),
                ]
    ...
    async def test_endpoint_recent_activity_last_7_days(self, app_with_router):
        ...
        def mock_execute(query):
            ...
            if "SECRET_CREATED" in query_str:
                ...
                activities = [
                    create_mock_activity(ActivityType.SECRET_CREATED, timestamp=now - timedelta(days=1)),
                    create_mock_activity(ActivityType.SECRET_CREATED, timestamp=now - timedelta(days=3)),
                    create_mock_activity(ActivityType.SECRET_CREATED, timestamp=now - timedelta(days=5)),
                    create_mock_activity(ActivityType.SECRET_CREATED, timestamp=now - timedelta(days=10)),
                ]
            elif "SECRET_DELETED" in query_str:
                # 2 recent
                activities = [
                    create_mock_activity(ActivityType.SECRET_DELETED, timestamp=now - timedelta(days=2)),
                    create_mock_activity(ActivityType.SECRET_DELETED, timestamp=now - timedelta(days=4)),
                ]
            else:
                activities = []
