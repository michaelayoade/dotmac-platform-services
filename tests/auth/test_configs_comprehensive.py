...
class TestRBACConfig:
    ...
    def test_env_overrides(self):
        ...
        with patch.dict(
            os.environ,
            {"RBAC_CACHE_TTL": "600", "RBAC_DEFAULT_ROLE": "member", "RBAC_MAX_ROLES": "5"},
        ):
    ...
    @pytest.mark.skip(
        reason="Pydantic ValidationError expectations conflict with config implementation"
    )
    def test_validation_errors(self):
        ...
        assert (
            "ensure this value is greater than" in str(exc_info.value).lower()
        ...
        assert (
            "bool" in str(exc_info.value).lower()
            or "validation error" in str(exc_info.value).lower()
        )
    ...
    def test_field_aliases_and_deprecation(self):
        ...
        config = RBACConfig(enable_rbac=False, enable_dynamic_permissions=True)
...
class TestSessionConfig:
    ...
    def test_cookie_security_settings(self):
        ...
        config = SessionConfig(cookie_secure=False, cookie_httponly=False, cookie_samesite="none")
    ...
    def test_ttl_and_limits(self):
        ...
        config = SessionConfig(session_ttl=3600, max_sessions_per_user=5)  # 1 hour
        assert config.session_ttl == 3600
        assert config.max_sessions_per_user == 5

        # Test edge cases
        config = SessionConfig(
            session_ttl=0, max_sessions_per_user=1  # No expiry  # Single session
        )
...
class TestMFAConfig:
    ...
    def test_totp_configuration(self):
        ...
        config = MFAConfig(
            totp_issuer="MyApp", totp_digits=8, totp_algorithm="SHA256", totp_window=2
        )
    ...
    def test_provider_configuration(self):
        ...
        config = MFAConfig(sms_provider="twilio", email_provider="sendgrid")
    ...
    def test_security_limits(self):
        ...
        config = MFAConfig(max_attempts=5, lockout_duration=1800)  # 30 minutes
        assert config.max_attempts == 5
        assert config.lockout_duration == 1800

        # Test edge cases
        config = MFAConfig(max_attempts=1, lockout_duration=0)  # Single attempt  # No lockout
        assert config.max_attempts == 1
        assert config.lockout_duration == 0

    def test_backup_codes_configuration(self):
        """Test backup codes settings."""
        config = MFAConfig(backup_codes_count=20, backup_code_length=12)
...
class TestOAuthConfig:
    ...
    def test_redirect_uri_configuration(self):
        ...
        config = OAuthConfig(redirect_uri_base="https://app.example.com")
        assert config.redirect_uri_base == "https://app.example.com"

        # Test with port
        config = OAuthConfig(redirect_uri_base="https://app.example.com:8443")
    ...
    def test_user_creation_settings(self):
        ...
        config = OAuthConfig(auto_create_users=False, require_email_verification=True)
    ...
    def test_secrets_redaction(self):
        ...
        config = OAuthConfig(
            google_client_secret="super-secret-key", github_client_secret="another-secret"
        )
...
class TestConfigEdgeCases:
    ...
    def test_very_large_values(self):
        ...
        config = RBACConfig(cache_ttl=2147483647, max_roles_per_user=1000)  # Max 32-bit int
        assert config.cache_ttl == 2147483647
        assert config.max_roles_per_user == 1000

        config = SessionConfig(session_ttl=31536000, max_sessions_per_user=100)  # 1 year in seconds
    ...
    def test_unicode_and_special_chars(self):
        ...
        config = RBACConfig(permission_delimiter="::", default_role="用户")  # Chinese for "user"
        assert config.permission_delimiter == "::"
    ...
    def test_json_serialization(self):
        ...
        config = SessionConfig(backend="redis", session_ttl=3600, cookie_secure=True)
