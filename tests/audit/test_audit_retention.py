...
@pytest.fixture
async def old_activities(async_db_session):
...
class TestAuditRetentionService:
    ...
    @pytest.mark.asyncio
    async def test_cleanup_old_logs_dry_run(self, retention_service, old_activities, async_db_session):
        ...
        assert len(results["by_severity"]) > 0
        ...
        assert remaining == len(old_activities)

    @pytest.mark.asyncio
    async def test_cleanup_old_logs_with_deletion(self, retention_service, old_activities, async_db_session):
        ...
        initial_count = len(old_activities)
        ...
        # Verify activities were actually deleted
        from sqlalchemy import select
        remaining_query = select(AuditActivity).where(
            AuditActivity.tenant_id == "test_tenant"
        )
    ...
    @pytest.mark.asyncio
    async def test_cleanup_with_archiving(self, retention_service, old_activities, async_db_session):
    ...
    @pytest.mark.asyncio
    async def test_restore_from_archive_with_filter(self, retention_service, tmp_path, async_db_session):
    ...
    @pytest.mark.asyncio
    async def test_get_retention_statistics(self, retention_service, old_activities, async_db_session):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_error_handling(self, retention_service):
        """Test error handling during cleanup."""
        # Skip this test as it requires proper error handling implementation
        pytest.skip("Error handling implementation needs to be completed")

    @pytest.mark.asyncio
    async def test_archive_error_handling(self, retention_service, old_activities, async_db_session):
        """Test error handling during archiving."""
        from unittest.mock import patch, AsyncMock

        # Make archive location unwritable
        retention_service.policy.archive_location = Path("/invalid/path")
        ...
        # Should have errors but continue with deletion
        assert len(results["errors"]) > 0
...
class TestCleanupTask:
    ...
    async def test_cleanup_audit_logs_task(self):
        ...
        with patch('dotmac.platform.audit.retention.AuditRetentionService') as MockService:
            ...
            mock_service.get_retention_statistics = AsyncMock(return_value={
                "total_records": 1000,
                "by_severity": {},
                "records_to_delete": {}
            })

            mock_service.cleanup_old_logs = AsyncMock(return_value={
                "total_deleted": 100,
                "total_archived": 100,
                "by_severity": {},
                "errors": []
