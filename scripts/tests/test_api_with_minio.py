...
async def test_api_with_minio():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL) as client:
        ...
        response = await client.post(
            "/api/v1/files/storage/upload", files=files, headers={"Authorization": TEST_TOKEN}
        )
        ...
        response = await client.get("/api/v1/files/storage", headers={"Authorization": TEST_TOKEN})
        ...
        response = await client.get(
            f"/api/v1/files/storage/{file_id}", headers={"Authorization": TEST_TOKEN}
        )
        ...
        response = await client.get(
            f"/api/v1/files/storage/{file_id}/download", headers={"Authorization": TEST_TOKEN}
        )
