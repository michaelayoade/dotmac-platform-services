...
CONFIG_PATTERNS = [
    ...
    (r"from \.\.config import (\w+Config)", "from dotmac.platform.settings import settings"),
    (
        r"from dotmac\.platform\.\w+\.config import (\w+Config)",
...
def update_file(filepath: Path) -> bool:
    ...
    try:
        ...
        for old_name, new_path in CONFIG_MAPPING.items():
            ...
            content = re.sub(
                rf"\b{old_name}\(([^)]+)\)", rf"{new_path}.model_copy(update={{\1}})", content
            )
...
def main():
    ...
    python_files = [
        f for f in python_files if f.name != "settings.py" and f.name != "migrate_to_settings.py"
    ]
