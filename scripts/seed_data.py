...
from src.dotmac.platform.billing.models import (
...
class DataSeeder:
    ...
    async def clear_existing_data(self):
        ...
        tables = [
            "invoices", "invoice_items", "subscriptions", "prices", "products",
            "customers", "secrets", "tenant_users", "users", "tenants",
        ...
        if user_list and tenant_list:
            ...
            if admin_user:
                for tenant in tenant_list:
                    await self.db.execute(
                        text("INSERT INTO tenant_users (tenant_id, user_id, role) VALUES (:tenant_id, :user_id, :role) ON CONFLICT DO NOTHING"),
    ...
    async def seed_secrets(self):
        ...
        for secret_data in TEST_SECRETS:
            ...
            existing = await self.db.execute(
                text("SELECT id FROM secrets WHERE path = :path"),
            ...
            if not existing.first():
                secret = Secret(
                    ...
                    path=secret_data["path"],
                    encrypted_value=str(secret_data["value"]),  # In real app, this would be encrypted
    ...
    async def seed_subscriptions(self):
        ...
        users = await self.db.execute(text("SELECT id, username FROM users WHERE username != 'admin'"))
        ...
        if user_list and product_list:
            ...
            for user_id, product_id, status in subscription_data:
                ...
                price_result = await self.db.execute(
                    text("SELECT id, amount FROM prices WHERE product_id = :product_id AND interval = 'month' LIMIT 1"),
