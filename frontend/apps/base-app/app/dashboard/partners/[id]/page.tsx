    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 mt-1">Partner #{partner.partner_number}</p>
          </div>
        </div>
        <button
      ...
      <div className="grid gap-4 md:grid-cols-4 mb-6">
        <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
          ...
          <div className="text-2xl font-bold text-white">{partner.total_customers}</div>
        </div>
        <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
      ...
      <div className="space-y-6">
        {activeTab === "overview" && (
          <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3 text-sm">
                {partner.legal_name && (
                  <div>
                    ...
                    <span className="ml-2 text-white">{partner.legal_name}</span>
                  </div>
                )}
                <div>
                  <span className="text-slate-400">Primary Email:</span>
                  <span className="ml-2 text-white">{partner.primary_email}</span>
                </div>
                {partner.billing_email && (
                  <div>
                    <span className="text-slate-400">Billing Email:</span>
                    <span className="ml-2 text-white">{partner.billing_email}</span>
                  </div>
                )}
                {partner.phone && (
                  <div>
                    <span className="text-slate-400">Phone:</span>
                    <span className="ml-2 text-white">{partner.phone}</span>
                  </div>
                )}
                {partner.website && (
                  <div>
            ...
            {/* Commission Details */}
            <div className="bg-slate-800 p-6 rounded-lg border border-slate-700">
              <h3 className="text-lg font-semibold text-white mb-4">Commission Details</h3>
              <div className="space-y-3 text-sm">
                <div>
            ...
            <div className="bg-slate-800 p-6 rounded-lg border border-slate-700">
              ...
              <div className="space-y-3 text-sm">
                <div>
                  ...
                  <span className="ml-2 text-white">{partner.total_referrals}</span>
                </div>
                <div>
                  <span className="text-slate-400">Converted Referrals:</span>
                  <span className="ml-2 text-white">{partner.converted_referrals}</span>
                </div>
                <div>
                  <span className="text-slate-400">Conversion Rate:</span>
                  <span className="ml-2 text-green-400">{conversionRate}%</span>
                </div>
                <div>
