  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const requestData: APIKeyCreateRequest = {
        ...
        expires_at: formData.never_expires ? undefined : formData.expires_at,
      };

      if (editingApiKey) {
        // Update existing API key
        await updateApiKey(editingApiKey.id, requestData);
        onApiKeyCreated();
      } else {
        // Create new API key
              ...
              {Object.entries(availableScopes).map(([scope, info]) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-sky-400 font-mono mt-1">{scope}</div>
                  </div>
                </label>
              ))}
            </div>
