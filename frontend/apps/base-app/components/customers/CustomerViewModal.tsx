...
import { X, User, Building, Mail, Phone, MapPin, Calendar, DollarSign, Hash, Globe } from 'lucide-react';
      ...
      <div className="bg-slate-900 rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center gap-4">
            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${statusColors[customer.status]}`}>
              {customer.status.charAt(0).toUpperCase() + customer.status.slice(1)}
            </span>
            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${tierColors[customer.tier]}`}>
          ...
          <div>
            ...
            <div className="bg-slate-800/50 rounded-lg p-4 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-slate-300">Customer #: {customer.customer_number}</span>
              </div>
              <div className="flex items-center gap-3">
                <User className="h-4 w-4 text-slate-500" />
                <span className="text-sm text-slate-300">Type: {customer.customer_type === 'individual' ? 'Individual' : 'Business'}</span>
              </div>
              {customer.tax_id && (
                <div className="flex items-center gap-3">
                  <Hash className="h-4 w-4 text-slate-500" />
            ...
            <div className="bg-slate-800/50 rounded-lg p-4 space-y-3">
              ...
              {customer.website && (
                <div className="flex items-center gap-3">
                  ...
                  <a href={customer.website} target="_blank" rel="noopener noreferrer" className="text-sm text-sky-400 hover:text-sky-300">
                    {customer.website}
                  </a>
                </div>
              )}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-slate-800/50 rounded-lg p-4">
                ...
                <div className="text-lg font-semibold text-white">{formatCurrency(customer.lifetime_value)}</div>
              </div>
              <div className="bg-slate-800/50 rounded-lg p-4">
          ...
          <div>
            ...
            <div className="bg-slate-800/50 rounded-lg p-4 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-slate-300">Created: {formatDate(customer.created_at)}</span>
              </div>
              <div className="flex items-center gap-3">
                <Calendar className="h-4 w-4 text-slate-500" />
                <span className="text-sm text-slate-300">Last Modified: {formatDate(customer.updated_at)}</span>
              </div>
              {customer.last_interaction && (
                <div className="flex items-center gap-3">
                  <Calendar className="h-4 w-4 text-slate-500" />
                  <span className="text-sm text-slate-300">Last Interaction: {formatDate(customer.last_interaction)}</span>
                </div>
              )}
            </div>
          </div>
