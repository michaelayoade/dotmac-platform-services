...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE saga_status_enum AS ENUM ('pending', 'running', 'completed', 'failed', 'compensating', 'compensated')"
    )
    op.execute(
        "CREATE TYPE step_status_enum AS ENUM ('pending', 'running', 'completed', 'failed', 'compensating', 'compensated', 'skipped')"
    )
    op.execute(
        "CREATE TYPE operation_status_enum AS ENUM ('pending', 'running', 'completed', 'failed')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("pending", "running", "completed", "failed", name="operation_status_enum"),
            nullable=False,
            default="pending",
            index=True,
        ),
...
def downgrade() -> None:
    ...
    # Drop triggers
    op.execute(
        "DROP TRIGGER IF EXISTS update_idempotent_operations_updated_at ON idempotent_operations"
    )
    op.execute(
        "DROP TRIGGER IF EXISTS update_saga_step_executions_updated_at ON saga_step_executions"
    )
