...
down_revision: str | None = "952b95951dab"
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_article_category_published", "knowledge_articles", ["category", "published_at"]
    )
    op.create_index(
        "ix_article_search_ranking", "knowledge_articles", ["search_ranking", "view_count"]
    )

    # Unique constraint for tenant + slug
    op.create_unique_constraint(
        "uq_article_slug_tenant", "knowledge_articles", ["tenant_id", "slug"]
    )
    ...
    op.create_unique_constraint(
        "uq_analytics_article_date", "article_analytics", ["article_id", "date"]
    )
    ...
    op.create_index(
        "ix_customer_portal_settings_customer_id", "customer_portal_settings", ["customer_id"]
    )
    op.create_index(
        "ix_customer_portal_settings_tenant_id", "customer_portal_settings", ["tenant_id"]
    )
    op.create_unique_constraint(
        "uq_customer_portal_settings", "customer_portal_settings", ["customer_id", "tenant_id"]
    )
    ...
    if not column_exists("tickets", "source"):
        op.add_column(
            "tickets", sa.Column("source", sa.String(), nullable=False, default="customer_portal")
        )
    ...
    if not column_exists("tickets", "external_references"):
        op.add_column(
            "tickets",
            sa.Column(
                "external_references", sa.JSON(), nullable=True, default=sa.text("'{}'::json")
            ),
        )
    ...
    tables_with_updated_at = ["knowledge_articles", "article_comments", "customer_portal_settings"]
