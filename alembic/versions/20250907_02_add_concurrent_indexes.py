...
def upgrade() -> None:
    ...
    _create_index(
        "ix_bl_incident_escalations_incident_id", "bl_incident_escalations", ["incident_id"]
    )
    ...
    _create_index(
        "ix_bl_payment_fraud_scores_payment_id", "bl_payment_fraud_scores", ["payment_id"]
    )
    ...
    _create_index(
        "ix_bl_provisioning_requests_tenant_status",
        "bl_provisioning_requests",
        ["tenant_id", "status"],
    )
    _create_index(
        "ix_bl_provisioning_requests_customer_id", "bl_provisioning_requests", ["customer_id"]
    )
    _create_index(
        "ix_bl_provisioning_requests_requested_at", "bl_provisioning_requests", ["requested_at"]
    )

    # bl_provisioning_validations
    _create_index(
        "ix_bl_provisioning_validations_request_id", "bl_provisioning_validations", ["request_id"]
    )
    _create_index(
        "ix_bl_provisioning_validations_check_name", "bl_provisioning_validations", ["check_name"]
    )
    ...
    # bl_workflow_state
    _create_index(
        "ix_bl_workflow_state_name_status", "bl_workflow_state", ["workflow_name", "status"]
    )
    ...
    # bl_workflow_history
    _create_index(
        "ix_bl_workflow_history_name_corr",
        "bl_workflow_history",
    ...
    # bl_workflow_metrics
    _create_index(
        "ix_bl_workflow_metrics_name_metric",
        "bl_workflow_metrics",
        ["workflow_name", "metric_name"],
    )
