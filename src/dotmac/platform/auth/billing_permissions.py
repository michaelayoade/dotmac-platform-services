"""Bootstrap tenant billing RBAC permissions and roles."""

from __future__ import annotations

from collections.abc import Iterable

from sqlalchemy import select
from sqlalchemy.ext.asyncio import AsyncSession

from dotmac.platform.auth.models import Permission, PermissionCategory, Role

# Tenant billing permissions
BILLING_PERMISSIONS: tuple[tuple[str, str, PermissionCategory], ...] = (
    # Subscription management
    ("billing.subscription.view", "View subscription details", PermissionCategory.BILLING),
    (
        "billing.subscription.manage",
        "Upgrade/downgrade subscription plans",
        PermissionCategory.BILLING,
    ),
    (
        "billing.subscription.cancel",
        "Cancel subscription",
        PermissionCategory.BILLING,
    ),
    # Add-on management
    ("billing.addons.view", "View available add-ons", PermissionCategory.BILLING),
    ("billing.addons.purchase", "Purchase and manage add-ons", PermissionCategory.BILLING),
    # Payment methods
    ("billing.payment_methods.view", "View saved payment methods", PermissionCategory.BILLING),
    (
        "billing.payment_methods.manage",
        "Add/remove payment methods",
        PermissionCategory.BILLING,
    ),
    # Invoices and payments
    ("billing.invoices.view", "View invoices and payment history", PermissionCategory.BILLING),
    ("billing.invoices.download", "Download invoices and receipts", PermissionCategory.BILLING),
    ("billing.invoices.manage", "Create and manage invoices", PermissionCategory.BILLING),
    ("billing.credit_notes.view", "View credit notes", PermissionCategory.BILLING),
    ("billing.credit_notes.manage", "Create and manage credit notes", PermissionCategory.BILLING),
    ("billing.payments.view", "View payments", PermissionCategory.BILLING),
    ("billing.payments.manage", "Manage payments", PermissionCategory.BILLING),
    ("billing.bank_accounts.view", "View bank accounts", PermissionCategory.BILLING),
    ("billing.bank_accounts.manage", "Manage bank accounts", PermissionCategory.BILLING),
    ("billing.receipts.view", "View receipts", PermissionCategory.BILLING),
    ("billing.receipts.download", "Download receipts", PermissionCategory.BILLING),
    ("billing.metrics.view", "View billing metrics", PermissionCategory.BILLING),
    ("billing.catalog.view", "View billing catalog", PermissionCategory.BILLING),
    ("billing.catalog.manage", "Manage billing catalog", PermissionCategory.BILLING),
    ("billing.pricing.view", "View pricing rules", PermissionCategory.BILLING),
    ("billing.pricing.manage", "Manage pricing rules", PermissionCategory.BILLING),
    ("billing.settings.view", "View billing settings", PermissionCategory.BILLING),
    ("billing.settings.manage", "Manage billing settings", PermissionCategory.BILLING),
    ("billing.dunning.view", "View dunning workflows", PermissionCategory.BILLING),
    ("billing.dunning.manage", "Manage dunning workflows", PermissionCategory.BILLING),
    # Usage monitoring
    ("billing.usage.view", "View usage metrics and balance", PermissionCategory.BILLING),
    ("billing.usage.manage", "Manage usage records", PermissionCategory.BILLING),
)


ROLE_PERMISSION_MAP: dict[str, Iterable[str]] = {
    # Tenant admins get all billing permissions
    "tenant_admin": [perm[0] for perm in BILLING_PERMISSIONS],
    # Platform admins get all permissions
    "admin": [perm[0] for perm in BILLING_PERMISSIONS],
    # Tenant billing managers get subset (no cancellation)
    "tenant_billing_manager": [
        "billing.subscription.view",
        "billing.subscription.manage",
        "billing.addons.view",
        "billing.addons.purchase",
        "billing.payment_methods.view",
        "billing.payment_methods.manage",
        "billing.invoices.view",
        "billing.invoices.download",
        "billing.invoices.manage",
        "billing.credit_notes.view",
        "billing.credit_notes.manage",
        "billing.payments.view",
        "billing.payments.manage",
        "billing.bank_accounts.view",
        "billing.bank_accounts.manage",
        "billing.receipts.view",
        "billing.receipts.download",
        "billing.metrics.view",
        "billing.catalog.view",
        "billing.catalog.manage",
        "billing.pricing.view",
        "billing.pricing.manage",
        "billing.settings.view",
        "billing.settings.manage",
        "billing.dunning.view",
        "billing.dunning.manage",
        "billing.usage.view",
        "billing.usage.manage",
    ],
    # Regular tenant users can only view
    "tenant_user": [
        "billing.subscription.view",
        "billing.addons.view",
        "billing.invoices.view",
        "billing.credit_notes.view",
        "billing.payments.view",
        "billing.bank_accounts.view",
        "billing.receipts.view",
        "billing.metrics.view",
        "billing.catalog.view",
        "billing.pricing.view",
        "billing.settings.view",
        "billing.dunning.view",
        "billing.usage.view",
    ],
}


async def _get_permission(session: AsyncSession, name: str) -> Permission | None:
    result = await session.execute(select(Permission).where(Permission.name == name))
    return result.scalars().first()


async def _get_role(session: AsyncSession, name: str) -> Role | None:
    result = await session.execute(select(Role).where(Role.name == name))
    return result.scalars().first()


async def ensure_billing_rbac(session: AsyncSession) -> None:
    """Ensure tenant billing permissions and role assignments exist."""

    created_permissions = []

    # Create permissions
    for name, description, category in BILLING_PERMISSIONS:
        permission = await _get_permission(session, name)
        if not permission:
            permission = Permission(
                name=name,
                display_name=name.replace(".", " ").replace("_", " ").title(),
                description=description,
                category=category,
                is_active=True,
                is_system=True,
            )
            session.add(permission)
            created_permissions.append(name)

    if created_permissions:
        await session.flush()

    role_cache: dict[str, Role] = {}

    # Create or get roles
    for role_name, _permissions in ROLE_PERMISSION_MAP.items():
        role = await _get_role(session, role_name)
        if not role:
            role = Role(
                name=role_name,
                display_name=role_name.replace("_", " ").title(),
                description=f"Default role for {role_name}",
                is_system=True,
                is_active=True,
            )
            session.add(role)
            await session.flush()
        role_cache[role_name] = role

    # Map permissions to roles
    for role_name, permissions in ROLE_PERMISSION_MAP.items():
        role = role_cache[role_name]
        await session.refresh(role, ["permissions"])
        existing = {perm.name for perm in role.permissions}
        to_add = []
        for perm_name in permissions:
            if perm_name in existing:
                continue
            permission = await _get_permission(session, perm_name)
            if permission:
                to_add.append(permission)
        if to_add:
            role.permissions.extend(to_add)
            await session.flush()

    await session.commit()
