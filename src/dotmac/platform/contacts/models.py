...
from sqlalchemy import (
    ...
    UniqueConstraint, Index, JSON, Integer, Table, CheckConstraint
)
...
contact_to_labels = Table(
    ...
    Column('label_definition_id', PGUUID(as_uuid=True), ForeignKey('contact_label_definitions.id', ondelete='CASCADE'), primary_key=True),
...
class Contact(Base):
    ...
    prefix: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)  # Mr, Ms, Dr, etc.
    suffix: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)  # Jr, Sr, III, etc.
    ...
    status: Mapped[ContactStatus] = mapped_column(String(50), default=ContactStatus.ACTIVE, nullable=False)
    stage: Mapped[ContactStage] = mapped_column(String(50), default=ContactStage.PROSPECT, nullable=False)
    ...
    anniversary: Mapped[Optional[DateTime]] = mapped_column(DateTime(timezone=True), nullable=True)

    # Flags
    is_primary: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)  # Primary contact for customer
    ...
    # Preferences
    preferred_contact_method: Mapped[Optional[ContactMethodType]] = mapped_column(String(50), nullable=True)
    preferred_language: Mapped[Optional[str]] = mapped_column(String(10), nullable=True)  # ISO 639-1 code
    ...
    deleted_by: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey('users.id'), nullable=True)

    # Relationships
    contact_methods = relationship("ContactMethod", back_populates="contact", cascade="all, delete-orphan")
    labels = relationship("ContactLabelDefinition", secondary=contact_to_labels, back_populates="contacts")
    ...
    __table_args__ = (
        ...
        CheckConstraint('display_name IS NOT NULL AND display_name != \'\'', name='check_display_name_not_empty'),
    )

    def __repr__(self) -> str:
        return f"<Contact(id={self.id}, display_name='{self.display_name}', company='{self.company}')>"
...
class ContactMethod(Base):
    ...
    value: Mapped[str] = mapped_column(String(500), nullable=False)  # Email, phone number, URL, etc.
    label: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)  # Work, Home, Mobile, etc.
    ...
    is_verified: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    is_public: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)  # Visible to all or internal only
...
class ContactFieldDefinition(Base):
    ...
    # Grouping
    field_group: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    ...
    is_encrypted: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)  # Encrypt in custom_fields
...
class ContactActivity(Base):
    ...
    # Activity details
    activity_type: Mapped[str] = mapped_column(String(50), nullable=False)  # call, email, meeting, note, etc.
    ...
    # Status
    status: Mapped[str] = mapped_column(String(50), nullable=False)  # scheduled, completed, cancelled
