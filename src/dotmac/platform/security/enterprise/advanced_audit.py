...
class AdvancedAuditLogger(AuditLogger):
    ...
    async def log_privacy_event(
        ...
        **kwargs,
        ...
        return await self.log_event(
            event_type=(
                AuditEventType.DATA_READ if "access" in event_type else AuditEventType.DATA_UPDATE
            ),
    ...
    async def generate_forensic_report(
        ...
        incident_events = [
            event
            for event in events
            if incident_id in str(event.custom_attributes.get("incident_id", ""))
        ]
        ...
        logger.info(
            "Forensic report generated", incident_id=incident_id, event_count=len(incident_events)
        )
        return report
