...
def get_audit_logger(
    *,
    ...
    """Return a shared audit logger instance (cached by default)."""

    global _DEFAULT_LOGGER
    if refresh or store is not None or tenant_id is not None or service_name != "dotmac-security":
        return AuditLogger(store=store, service_name=service_name, tenant_id=tenant_id)

    if _DEFAULT_LOGGER is None:
        _DEFAULT_LOGGER = AuditLogger(store=store, service_name=service_name, tenant_id=tenant_id)
...
async def record_audit_event(
    ...
    **kwargs: Any,
    ...
    """Convenience helper to record an audit event using the shared logger."""

    logger = get_audit_logger(service_name=service_name, tenant_id=tenant_id, store=store)
    return await logger.log_event(event_type=event_type, message=message, **kwargs)
